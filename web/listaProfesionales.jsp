<%-- 
    Document   : listaProfesionales
    Created on : 06-nov-2019, 12:54:14
    Author     : Dell
--%>
...
<br><br><br>
        <form method="POST" action="/centroMedicinalDeDeporte/servletListaProfesionales" onsubmit="return validar();">

            
                                    <table border="1" align="center" class="table table-striped">
                                        <tr><th> N° </th><th>Profesional</th><th>Tipo</th><th>DNI</th><th>Sexo</th><th>Telefono</th><th>Direccion</th><th>Email</th><th>1</th><th>2</th> <tr>
					<c:forEach items="${listaProfesionales}" var="p">
                                            <tr>   
                                                <td> ${p.getLegajo()}</td>
                                                <td> ${p.toString()} </td>
                                                <td> ${p.getTipo().getTipoProfesional()}  </td>
                                                <td> ${p.getDni()}  </td>
                                                <td> ${p.getSexo()} </td>
                                                <td> ${p.getTelefono()} </td>
                                                <td> ${p.getDomicilio()} </td>
                                                <td> ${p.getEmail()}  </td>
                                                <td> <a href="/centroMedicinalDeDeporte/servletAltaProfesional?legajo=${p.legajo}">modificar</a></td>
                                                <td> <a href="/centroMedicinalDeDeporte/servletEliminarProfesional?legajo=${p.legajo}">eliminar</a></td>
